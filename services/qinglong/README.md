...
青龙面板：定时任务（Python / JS / Shell / TS）。仅经 Nginx 对外。
...
走路径前缀时，`QINGLONG_BASE_URL` 必须与 Nginx `location` 一致（例如都是 `/ql/`）。

## 事后核对清单

1. `PROFILES` 含 `qinglong`  
2. DNS / Nginx：`qinglong.${DOMAIN}`；`QINGLONG_BASE_URL` 与访问路径一致  
